...
class AnnotationsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "can create example answer and guidance at the same time" do
    ...
    assert_redirected_to "#{org_admin_template_phase_path(@question.section.phase.template.id, @question.section.phase.id)}?section_id=#{@question.section.id}&r=all-templates"
