...
class DmpIdService
  ...
  class << self
    ...
    def mint_dmp_id(plan:)
      ...
      Rails.logger.error "DoiService.mint_dmp_id for Plan #{plan&.id} resulted in: #{e.message}"
      nil
    end
    ...
    def update_dmp_id(plan:)
      ...
      Rails.logger.error "DoiService.update_dmp_id for Plan #{plan&.id} resulted in: #{e.message}"
