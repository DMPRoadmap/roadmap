...
class Plan < ApplicationRecord
  ...
  def dmp_id
    ...
    # This is here in the event that the DmpIdService has changed over time and the
    # Plan's DMP ID was generated by an older service
    identifiers.select { |i| %w[ark doi].include?(i.identifier_format) }.last
  end
  ...
  def unique_identifier(identifier_scheme:)
    # If the system has the DMP ID gem installed use the plan's DMP ID
    dmp_id = plan.dmp_id if plan.respond_to?(:dmp_id)
