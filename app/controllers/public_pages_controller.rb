class PublicPagesController < ApplicationController
  after_action :verify_authorized, except: [:template_index, :plan_index, :institutions]

  def institutions
    render 'institutions', locals: {orgs: Org.includes(:identifier_schemes).where('is_other IS NULL OR is_other = 0').order(:name)}
  end
    ...
    begin
      ...
      respond_to do |format|
        ...
        format.pdf do
          ...
          footer: {
            center:    _('Template created using the %{application_name}. Last modified %{date}') % {application_name: Rails.configuration.branding[:application][:name], date: @template.updated_at},
            font_size: 8,
