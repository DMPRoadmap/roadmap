    ...
    class PlansController < BaseApiController
      ...
      def create
        ...
        if dmp[:dmp_id].present? && dmp[:dmp_id][:identifier].present?
          ...
          dmp_id = Identifier.where(value: dmp[:dmp_id][:identifier], identifier_scheme: scheme)
        end
        ...
        if dmp_id.present?
          ...
          if plan.present?
            if plan.created_at.utc < now.utc
              ...
              # associate the Plan to the ApiClient.org or the User.org depending
              # on who the caller is and if they have an Org
              plan.update(org: client.org) if client.org.present?
