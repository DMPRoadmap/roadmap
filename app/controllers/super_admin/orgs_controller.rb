  ...
  class OrgsController < ApplicationController
    ...
    def create
      ...
      begin
        ...
        if org.save
          redirect_to admin_edit_org_path(org.id), notice: success_message(_('created'), _('organisation'))
        else
          flash.now[:alert] = failure_message('create', _('organisation'))
          render 'orgs/admin_edit', locals: {org: org, languages: Language.all.order("name"),
                                             method: 'POST', url: super_admin_orgs_path }
        end
    ...
    def destroy
      ...
      unless org.users.length > 0 || org.templates.length > 0
        ...
        if org.destroy!
          ...
          redirect_to super_admin_orgs_path, alert: failure_message('remove', _('organisation'))
        end
      end
    end
