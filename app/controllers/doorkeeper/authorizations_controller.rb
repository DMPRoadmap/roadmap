...
p "---------------------------"
p "NEW"
p pre_auth.authorizable?
p pre_auth.response_type
p pre_auth.resource_owner&.inspect
p "------------------"
    ...
    def render_success
p "---------------------------"
p "SKIP? #{skip_authorization?} || #{matching_token?}"
p "---------------------------"
    ...
    def matching_token?
      # DMPTool Customization to prevent the use of an AccessToken that has no
      # ResourceOwner (aka User) when this an authorization_code grant flow
      return nil if pre_auth.response_type == "code" && current_resource_owner.nil?
...
p "---------------------------"
p "AUTHORIZE RESPONSE"
p @authorize_response
p "---------------------------"
