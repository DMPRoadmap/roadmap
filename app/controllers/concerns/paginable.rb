module Paginable
  ...
  included do
    ...
    def paginable_renderise(partial: nil, controller: params[:controller], action: params[:action], scope: nil, locals: {})
      ...
      if params[:page] == 'ALL'
        # url to View Less records from the scope
        url = url_for(controller: controller, action: action, page: 1) 
      else
        # url to View All records from the scope
        url = url_for(controller: controller, action: action, page: 'ALL')
      end
