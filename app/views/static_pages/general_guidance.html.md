...
Today, this means data identifiers should fit inside a web address (URL) and be managed to remain actionable over the long term. The most important factors in long-term data sharing are stable data storage and well-managed identifier redirection.
...
- [**Handle**](http://www.handle.net/) – an identifier that becomes actionable when embedded in a URL. Handles are resolved at "http://handle.net". Resolution depends on HTTP redirection and the Handle protocol, and can be managed through an API or a user interface. Annual fees apply to each local Handle server.   
...
- **Institutional**: accepts data of any type produced within the institution that maintains it (e.g., the University of California's [Dash](https://dash.ucop.edu/stash))   
