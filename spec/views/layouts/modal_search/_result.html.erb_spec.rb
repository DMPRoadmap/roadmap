...
describe 'layouts/modal_search/_result.html.erb' do
  ...
  it "displays the result's :item_name_attr" do
    ...
    expect(rendered.include?('modal-search-result-selector visually-hidden-focusable')).to eql(true)
    expect(rendered.include?('modal-search-result-unselector visually-hidden-focusable')).to eql(false)
  end
  ...
  it "shows the 'Select' button and hides the 'Remove' button when :selected is false" do
    ...
    expect(rendered.include?('modal-search-result-selector visually-hidden-focusable')).to eql(false)
    expect(rendered.include?('modal-search-result-unselector visually-hidden-focusable')).to eql(true)
  end
end
