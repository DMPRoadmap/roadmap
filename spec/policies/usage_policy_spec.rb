...
RSpec.describe UsagePolicy, type: :policy do
  ...
  context 'super_admin' do
    ...
    it 'has access to all actions' do
      expect(subject).to permit_actions(super_actions)
    end
  end
  ...
  context 'org_admin' do
    ...
    it 'has access to all org-admin actions' do
      expect(subject).to permit_actions(org_actions)
    end

    it 'does not have access to global statistics' do
      expect(subject).to forbid_actions(%i[global_statistics])
    end
  end
  ...
  context 'user' do
    ...
    it 'not have access to any of the actions' do
      expect(subject).to forbid_actions(super_actions)
    end
  end
  ...
  context 'unauthenticated' do
    ...
    it 'not have access to any of the actions' do
      super_actions.each do |action|
        ...
        expect { expect(subject).to permit_action(action) }.to raise_error(Pundit::NotAuthorizedError), "expected :#{action} to raise a NotAuthorizedError"
