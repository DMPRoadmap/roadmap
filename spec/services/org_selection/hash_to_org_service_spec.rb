...
RSpec.describe OrgSelection::HashToOrgService do
  ...
  describe '#to_org(hash:)' do
    it 'returns nil if the hash is empty' do
      expect(described_class.to_org(hash: nil)).to be_nil
    end
    ...
    it 'returns a new Org instance' do
      expect(described_class.to_org(hash: @hash).new_record?).to be(true)
    end
  end
  ...
  describe '#to_identifiers(hash:)' do
    ...
    it 'returns an array of new Identifiers' do
      expect(@rslt.is_a?(Array)).to be(true)
      expect(@rslt.length).to be(1)
    end
    ...
    it 'returned Identifiers have a value' do
      expect(@rslt.first.value.ends_with?(@hash[:"#{@scheme.name}"])).to be(true)
    end
  ...
  context 'private methods' do
    describe '#initialize_org(hash:)' do
      ...
      it 'returns a new instance of Org' do
        ...
        expect(rslt.is_a?(Org)).to be(true)
        ...
        expect(rslt.institution?).to be(true)
    ...
    describe '#identifier_keys' do
      ...
      it 'returns the identifier key' do
        expect(@rslt.include?(@scheme.name.to_s)).to be(true)
      end

      it 'does not return the other keys' do
        expect(@rslt.include?('name')).to be(false)
        expect(@rslt.include?('sort_name')).to be(false)
        expect(@rslt.include?('weight')).to be(false)
        expect(@rslt.include?('score')).to be(false)
        expect(@rslt.include?('language')).to be(false)
        expect(@rslt.include?('url')).to be(false)
        expect(@rslt.include?(@attr_key.to_s)).to be(false)
      end
    end
    ...
    describe '#attr_keys(hash:)' do
      ...
      it 'does not include identifier keys' do
        expect(@rslt.include?(@scheme.name.to_s)).to be(false)
      end

      it 'returns the other attributes' do
        expect(@rslt.include?('name')).to be(true)
        expect(@rslt.include?('language')).to be(true)
        expect(@rslt.include?('url')).to be(true)
        expect(@rslt.include?(@attr_key.to_s)).to be(true)
      end
    end
