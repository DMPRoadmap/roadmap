...
RSpec.describe ExternalApis::Re3dataService do
  ...
  describe "#fetch" do
    before(:each) do
      ...
      @list_path = "#{described_class.api_base_url}#{described_class.list_path}"
      ...
      @repositories_results = <<-XML
        <list>
          <repository>
            <id>r3d100000044</id>
            <name>DRYAD</name>
            <link href="/api/v1/repository/#{@repo_id}" rel="self"/>
          </repository>
        </list>
        XML
  ...
  context "private methods" do
    describe "#query_re3data" do
      ...
      it "returns the response body as XML" do
        ...
        expected = Nokogiri::XML(@repositories_results, nil, "utf8")
        expect(described_class.send(:query_re3data)).to eql(expected)
      end
    end
    ...
    describe "#query_re3data_repository(repo_id:)" do
      ...
      it "returns the response body as JSON" do
        ...
        expected = Nokogiri::XML(@repository_result, nil, "utf8")
        expect(described_class.send(:query_re3data_repository, repo_id: @repo_id)).to eql(expected)
      end
    end
    ...
    describe "#process_repository(id:, node:)" do
      ...
      it "returns nil if :id is not present" do
        expect(described_class.send(:process_repository, id: nil, node: @node)).to eql(nil)
      end
      ...
      it "finds an existing Repository by its identifier" do
        ...
        expect(described_class.send(:process_repository, id: @repo_id, node: @node)).to eql(repo)
      end
      ...
      it "creates a new Repository" do
        repo = described_class.send(:process_repository, id: @repo_id, node: @node)
        expect(repo.new_record?).to eql(false)
        expect(repo.name).to eql(@node.xpath("//r3d:repositoryName")&.text)
      end
      it "attaches the identifier to the Repository (if it is not already defined" do
        repo = described_class.send(:process_repository, id: @repo_id, node: @node)
        expect(repo.identifiers.last.value).to eql(@repo_id)
      end
    end
    ...
    describe "#parse_repository(repo:, node:)" do
      ...
      context ":info JSON content" do
        ...
        it "updates the :policies" do
          described_class.send(:parse_repository, repo: @repo, node: @node)
          expect(@repo.info[:policies]).to eql(@expected[:policies])
        end
        it "updates the :upload_types" do
          described_class.send(:parse_repository, repo: @repo, node: @node)
          expect(@repo.info[:upload_types]).to eql(@expected[:upload_types])
        end
      end
    end
