...
RSpec.describe Api::V2::PersistenceService do
  describe 'safe_save(plan:)' do
    it 'returns nil if :plan is not a Plan' do
      expect(described_class.safe_save(plan: build(:org))).to be_nil
    end

    it 'returns nil if :plan is not valid?' do
      expect(described_class.safe_save(plan: build(:plan, title: nil))).to be_nil
    end
    ...
    it 'saves the :plan that has contributors' do
      ...
      expect(result.contributors.first.new_record?).to be(false)
    ...
    it 'saves the :plan that has identifiers' do
      ...
      expect(result.identifiers.first.new_record?).to be(false)
    ...
    it 'saves the :plan that has a funder' do
      ...
      plan.funder = org
      result = described_class.safe_save(plan: plan)
      expect(result.funder.present?).to be(true)
    ...
    it 'saves the :plan that has a grant' do
      ...
      plan.grant = id
      result = described_class.safe_save(plan: plan)
      expect(result.grant.present?).to be(true)
  ...
  context 'private methods' do
    describe 'safe_save_identifier(identifier:)' do
      ...
      it 'returns nil unless :identifier is an Identifier' do
        expect(described_class.send(:safe_save_identifier, identifier: build(:org))).to be_nil
      end

      it 'returns nil and does not save the :identifier if it is not valid' do
        id = build(:identifier, identifiable: @org, value: nil)
        expect(described_class.send(:safe_save_identifier, identifier: id)).to be_nil
      end
    ...
    describe 'safe_save_org(org:)' do
      it 'returns nil unless :org is an Org' do
        expect(described_class.send(:safe_save_org, org: build(:identifier))).to be_nil
      end

      it 'returns nil and does not save the :org if it is not valid' do
        org = build(:org, name: nil)
        expect(described_class.send(:safe_save_org, org: org)).to be_nil
      end
    ...
    describe 'safe_save_contributor(contributor:)' do
      it 'returns nil unless :contributor is an Contributor' do
        expect(described_class.send(:safe_save_contributor, contributor: build(:org))).to be_nil
      end
      ...
      it 'returns nil and does not save the :contributor if it is not valid' do
        ...
        expect(described_class.send(:safe_save_contributor, contributor: contributor)).to be_nil
      end
