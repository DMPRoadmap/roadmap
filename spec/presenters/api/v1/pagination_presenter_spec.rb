...
RSpec.describe Api::V1::PaginationPresenter do
  describe '#url_without_pagination' do
    ...
    it 'returns nil if no url was specified' do
      ...
      expect(presenter.url_without_pagination).to be_nil
    end
  ...
  describe '#prev_page?' do
    it 'returns false if we are on page 1' do
      ...
      expect(presenter.prev_page?).to be(false)
    end
    ...
    it 'returns true if more than 1 page and we are not on page 1' do
      ...
      expect(presenter.prev_page?).to be(true)
    end
  end
  ...
  describe '#prev_page_link' do
    ...
    it 'includes per_page in the query string' do
      expect(@presenter.prev_page_link.include?('per_page=2')).to be(true)
    end

    it 'includes shows the correct page number' do
      expect(@presenter.prev_page_link.include?('page=1')).to be(true)
    end

    it 'retains other query params' do
      expect(@presenter.prev_page_link.include?('other=true')).to be(true)
    end
  end
  ...
  describe '#next_page_link' do
    ...
    it 'includes per_page in the query string' do
      expect(@presenter.next_page_link.include?('per_page=2')).to be(true)
    end

    it 'includes shows the correct page number' do
      expect(@presenter.next_page_link.include?('page=2')).to be(true)
    end

    it 'retains other query params' do
      expect(@presenter.next_page_link.include?('other=true')).to be(true)
    end
  end
  ...
  context 'private methods' do
    describe '#total_pages' do
      it 'returns 1 if total_items is missing' do
        presenter = described_class.new(current_url: nil, per_page: 2,
                                        total_items: nil)
        expect(presenter.send(:total_pages)).to be(1)
      end
      ...
      it 'returns the total_items / per_page' do
        presenter = described_class.new(current_url: nil, per_page: 2,
                                        total_items: 4)
        expect(presenter.send(:total_pages)).to be(2)
      end
