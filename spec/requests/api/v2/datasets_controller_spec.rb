...
RSpec.describe Api::V2::DatasetsController, type: :request do
  ...
  describe 'POST /api/v2/plans/[:id]/datasets - create' do
    ...
    it 'skips datasets that already exist' do
      ...
      create(:identifier, identifiable: research_output, value: @dataset_json[:dataset_id][:identifier])
      @plan.reload
      post api_v2_datasets_path(@plan), params: @json.to_json
      expect(response.code).to eql('400')
      expect(response).to render_template('api/v2/error')
    ...
    context 'ResearchOutput inspection' do
      ...
      it 'set the dataset fields' do
        ...
        expected = Api::V1::ConversionService.yes_no_unknown_to_boolean(@dataset_json[:personal_data])
        expect(@research_output.personal_data).to eql(expected)
        expected = Api::V1::ConversionService.yes_no_unknown_to_boolean(@dataset_json[:sensitive_data])
        expect(@research_output.sensitive_data).to eql(expected)
