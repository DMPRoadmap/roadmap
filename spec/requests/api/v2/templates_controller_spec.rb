...
RSpec.describe Api::V2::TemplatesController, type: :request do
  ...
  describe "GET /api/v2/templates (index)" do
    ...
    it "returns the expected templates when ApiClient has a User.org association" do
      ...
      public_unpublished = create(:template, visibility: 1, published: false, org: create(:org))

      # Organisationally visible templates owned by another Org
      org_published = create(:template, visibility: 0, published: true, org: create(:org))
      org_unpublished = create(:template, visibility: 0, published: false, org: create(:org))
      ...
      my_org_unpublished = create(:template, visibility: 0, published: false, org: @org)

      # Customizations
      org_customization = create(:template, visibility: 1, published: true, org: create(:org),
