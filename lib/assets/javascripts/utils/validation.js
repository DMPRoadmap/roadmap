...
const blockHelp = (id, msg) => {
  if (isString(id) && isString(msg)) {
    return `<span id="${id}" class="help-block red" style="display:none;">${msg}</span>`;
  }
  return '';
};
const addAsterisk = (el) => {
  const asterisk = `<span class="red" title="${getConstant('VALIDATION_MESSAGE_TEXT')}">* </span>`;
  if (isObject(el)) {
...
const removeAsterisk = (el) => {
  if (isObject(el)) {
    const asterisk = el.closest('.form-group, .checkbox').find(`span[title="${getConstant('VALIDATION_MESSAGE_TEXT')}"]`);
    if (isObject(asterisk)) {
      asterisk.remove();
    }
  }
};
...
const valid = (el) => {
  ...
  $(el).closest('.form-group, .checkbox').find('.help-block').hide();
};
const invalid = (el) => {
  $(el).parent().addClass(validationStates.hasError);
  $(el).attr(ariaInvalid(true));
  $(el).closest('.form-group, .checkbox').find('.help-block').show();
};
const addValidationMessage = (el, excludeAsterisks) => {
  ...
  if (isString(target.attr('id'))) {
    ...
    if (!isString(target.attr('aria-describedby'))) {
      target.closest('.form-group, .checkbox').append(blockHelp(`help-${id}`, getValidationMessage(el)));
