...
console.log(isString($(el).attr('aria-describedby')));
...
const checkValidations = (el) => {
  ...
  // If the field is required or it has a value
...
export const isValidatable = (ctx) => {
  if (isObject(ctx)) {
    return isString($(ctx).attr('data-validatable'));
  }
  return false;
};
