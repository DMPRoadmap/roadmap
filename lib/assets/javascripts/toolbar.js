$(document).ready(function() {
  ...
  function buildSettingsForm(data) {
    ...
    table.after('<p>' + __('The items you select here will be displayed in the table below. You can sort the data by each of these headings or filter by entering a text string in the search box.') + '</p>');

    // Default name column
    table.before('<input type="hidden" name="columns[name]" value="1" />');
    thead.append('<th><label for="columns_name">' + __('Name') + '</label></th>');
    ...
    for (var i = 0, len = cols.length; i < len; i++) {
      var title = cols[i].replace(/^\w|_/g, function(c) { return c === '_' ? ' ' : c.toUpperCase(); }),
          label = $('<th><label for="columns_' + cols[i] + '">' + __('' + title) + '</label></th>').appendTo(thead),
    ...
    thead.append('<th><label for="columns_select">' + __('Select an action') + '</label></th>');
    tbody.append('<td><input type="checkbox" id="columns_select" name="columns[select]" value="1" checked disabled /></td>')

    var submit = $('<td><input type="Submit" value="' + __('Save') + '" class="btn btn-primary" /></td>').appendTo(tfoot);
        cancel = $('<td><a href="#" class="btn btn-primary">' + __('Cancel') + '</a></td>').appendTo(tfoot);
