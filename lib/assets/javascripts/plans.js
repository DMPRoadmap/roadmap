   ...
   //reload page back to where it was before committing comment
	...
	window.onbeforeunload = function(){
		...
		if ($.fn.is_dirty()) {
			...
			message = _('You have unsaved answers in the following sections:')
...
$.fn.update_plan_progress = function(data) {
	...
	$("#questions-progress-title").text(data.num_answers+"/"+data.num_questions + " " + _('questions answered'));
	$('#export-progress').css('width', data.space_used + '%');
	$("#export-progress-title").text(_('approx. %{space_used}% of available space used', {space_used: data.space_used}));
...
$.fn.update_question_timestamp = function(question_id) {
	...
	if (timestamp != null) {
        ...
        q_status.text("");
        q_status.append( _('Answered') + " <abbr class='timeago'></abbr> " + _(' by ') + data.questions[question_id]["answered_by"]);
...
$.fn.check_section_lock = function() {
	...
	$.getJSON("locked?section_id="+section_id, function(data) {
		if (data.locked) {
			section.find(".section-lock-notice").html("<p>" + _('This section is locked for editing by ') + data.locked_by + ".</p>");
