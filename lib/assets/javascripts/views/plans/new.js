...
import { NO_TEMPLATE_FOUND_ERROR, NEW_PLAN_DISABLED_TOOLTIP } from '../../constants';
  ...
  const toggleSubmit = () => {
    ...
    if (isString(tmplt)) {
      ...
      $('#new_plan button[type="submit"]').attr('disabled', true)
        .attr('data-toggle', 'tooltip').attr('title', NEW_PLAN_DISABLED_TOOLTIP);
    }
  };

  // AJAX error function for available template search
  const error = () => {
    renderAlert(NO_TEMPLATE_FOUND_ERROR);
  };
