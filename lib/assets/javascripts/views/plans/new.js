$(document).ready(function(){
  ...
  $("#plan_funder_name").blur(function(){
    var msg = ($("#plan_funder_id").val().trim().length <= 0 ? __('Please select a valid funding organisation from the list') : '');
    toggleAutocompleteError($(this), $("#plan_funder_id"), msg);
  });
  $("#plan_org_name").blur(function(){
    var msg = ($("#plan_org_id").val().trim().length <= 0 ? __('Please select a valid research organisation from the list') : '');
    toggleAutocompleteError($(this), $("#plan_org_id"), msg);
  });
