...
$(() => {
  ...
  $(parentSelector).on('ajax:before', 'a.ajaxified-link[data-remote="true"]', (e) => {
    const panelBody = $(e.target).parent().find('.panel-body');
    return panelBody.attr('data-loaded') === 'false';
  });
  $(parentSelector).on('ajax:success', 'a.ajaxified-link[data-remote="true"]', (e, data) => {
    const panelBody = $(e.target).parent().find('.panel-body');
  ...
  $(parentSelector).on('ajax:error', 'a.ajaxified-link[data-remote="true"]', (e) => {
    const panelBody = $(e.target).parent().find('.panel-body');
  ...
  // NOT NEEDED SINCE THE SECTION WILL BE LOADED IN SHOW MODE
