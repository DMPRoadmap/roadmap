...
namespace :upgrade do
  ...
  task migrate_other_organisation_to_org: :environment do
    names = Users.all.where.not(other_organisation: nil).pluck(:other_organisation).uniq
    names.each do |name|
      suggestions = OrgSelectionService.search(
        search_term: name,
        include_locals: true,
        include_externals: true
      )
      match = suggestions.select { |hash| hash[:weight] < 2 }.first
      next unless match.present?

      # Found a match to an existing Org so just use that Id
      if match[:id].blank?
        org_id = match[:id]
      else
        # Found a match in ROR so create the Org and use its new id
        org = OrgSelectionService.convert_hash_to_org(hash: match)
        org.save if org.new_record?
        org_id = Org.where(name: match[:name]).first&.id
      end
      next unless org_id.present?

      # Connect any users with that other_organisation to the Org
      User.where(other_organisation: name).update_all(org_id: org_id)
    end
  end
