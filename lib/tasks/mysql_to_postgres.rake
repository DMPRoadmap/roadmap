...
namespace :mysql_to_postgres do
    ...
    task :read_3 => :environment do
        ...
        ActiveRecord::Base.connection.exec_query(sql).map do |question|
            ...
            p "question: id is "+ question['id'].to_s
            p "question: section_id is "+  question['section_id'].to_s
        ...
        ActiveRecord::Base.connection.exec_query(sql).map do |question_option|
            ...
            timestamps.each do |ek|
                if question_option[ek.to_sym].present? 
                    question_option[ek.to_sym] =  question_option[ek.to_sym].to_s 
                else 
                    question_option[ek.to_sym] = nil 
                end
            end
